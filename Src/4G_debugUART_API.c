...
sim_res_db_t sim_wait_result_db(UART_HandleTypeDef *uart_sim, uint32_t total_ms)
{
  ...
  while (HAL_GetTick() - t0 < total_ms){
    if (HAL_UART_Receive(uart_sim, &b, 1, 80) == HAL_OK){
      ...
      if (strstr(win, "+CMGS:") || strstr(win, "\nOK")) return SIM_RES_OK_DB;
      if (strstr(win, "+CME ERROR")) return SIM_RES_CME_DB;
      if (strstr(win, "+CMS ERROR")) return SIM_RES_CMS_DB;
      if (strstr(win, "ERROR"))      return SIM_RES_ERROR_DB;
    }
  }
  return SIM_RES_TIMEOUT_DB;
}
...
void sim_cmd_expect_ok_db(UART_HandleTypeDef *uart_sim, UART_HandleTypeDef *uart_dbg, const char *cmd, uint32_t wait_ms){
  ...
  sim_res_db_t r = sim_wait_result_db(uart_sim, wait_ms);
  if (r == SIM_RES_OK_DB)
	  dbg_puts(uart_dbg, "OK\r\n");
  else if (r == SIM_RES_CME_DB)
	  dbg_puts(uart_dbg, "CME ERROR\r\n");
  else if (r == SIM_RES_CMS_DB)
	  dbg_puts(uart_dbg, "CMS ERROR\r\n");
  else if (r == SIM_RES_ERROR_DB)
...
void buildPDU_db(const char* phone, const char* text, char *out_pdu, size_t out_sz){
  ...
  utf8ToUcs2Hex_db(text, ucs2, sizeof(ucs2));

  char numPDU[40];

  uint8_t numlen=0;

  encodePhoneNumberPDU_db(phone, numPDU, sizeof(numPDU), &numlen);
...
void sim_init_db(UART_HandleTypeDef *uart_sim, UART_HandleTypeDef *uart_dbg){
  HAL_Delay(2500);
  sim_cmd_expect_ok_db(uart_sim, uart_dbg, "AT",        1500);
  sim_cmd_expect_ok_db(uart_sim, uart_dbg, "ATE0",      1500);
  sim_cmd_expect_ok_db(uart_sim, uart_dbg, "AT+CMGF=0", 1500);
}
...
{
  char pdu[640];
  buildPDU_db(phone, text, pdu, sizeof(pdu));
  ...
  if (!sim_wait_prompt_db(uart_sim, 5000)){
    dbg_puts(uart_dbg, "NO_PROMPT\r\n");

    (void)sim_wait_result_db(uart_sim, 3000);

    sim_cmd_expect_ok_db(uart_sim, uart_dbg, "AT", 1000);
    HAL_Delay(300);
    return;
  }
  ...
  sim_res_db_t r = sim_wait_result_db(uart_sim, 15000);
  if (r == SIM_RES_OK_DB)          dbg_puts(uart_dbg, "OK\r\n");
  else if (r == SIM_RES_CMS_DB)    dbg_puts(uart_dbg, "CMS ERROR\r\n");
  else if (r == SIM_RES_CME_DB)    dbg_puts(uart_dbg, "CME ERROR\r\n");
  else if (r == SIM_RES_ERROR_DB)  dbg_puts(uart_dbg, "ERROR\r\n");
