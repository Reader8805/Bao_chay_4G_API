...
void sim_wait_result(UART_HandleTypeDef *uart_sim, uint32_t total_ms)
{
  ...
  while (HAL_GetTick() - t0 < total_ms){
	  HAL_UART_Receive(uart_sim, &b, 1, 120);
//    uint8_t b;
//    if (HAL_UART_Receive(uart_sim, &b, 1, 120) == HAL_OK){
//
//      if (wr+1 < sizeof(win))
//      {
//    	  win[wr++] = (char)b; win[wr] = 0;
//      }
//      else {
...
//      if (strstr(win, "\r\nOK\r\n") || strstr(win, "\nOK\r\n") || strstr(win, " OK\r\n"))
//    	  return SIM_RES_OK;
//      if (strstr(win, "+CME ERROR"))
//    	  return SIM_RES_CME;
//      if (strstr(win, "+CMS ERROR"))
//    	  return SIM_RES_CMS;
//      if (strstr(win, "\r\nERROR\r\n") || strstr(win, "\nERROR\r\n") || strstr(win, " ERROR\r\n"))
...
bool sim_wait_prompt(UART_HandleTypeDef *uart_sim, uint32_t max_ms){
  ...
  while (HAL_GetTick() - t0 < max_ms){
    ...
    if (HAL_UART_Receive(uart_sim, &ch, 1, 80) == HAL_OK && ch == '>')
    	return true;
  }
  return false;
}
...
void sim_cmd_expect_ok(UART_HandleTypeDef *uart_sim, /*UART_HandleTypeDef *uart_dbg, */const char *cmd, uint32_t wait_ms){
  ...
  sim_wait_result(uart_sim, wait_ms);
  //if (r == SIM_RES_OK);
...
void utf8ToUcs2Hex(const char* u8, char *out_hex, size_t out_sz){
  size_t wr=0; out_hex[0]='\0';
  while (*u8 && wr+4 < out_sz){
    uint16_t cp=0x003F; uint8_t c=(uint8_t)*u8++;
    if (c<0x80) cp=c;
    else if((c&0xE0)==0xC0 && (u8[0]&0xC0)==0x80){
    	cp=((c&0x1F)<<6)|(u8[0]&0x3F); u8++;
    }
...
void send_SMS(UART_HandleTypeDef *uart_sim, /*UART_HandleTypeDef *uart_dbg, */const char *phone, const char *text){
  char pdu[640]; buildPDU(phone, text, pdu, sizeof(pdu));
  size_t total_bytes = strlen(pdu)/2;
  if (!total_bytes)
	  return;
  int tpdu_len = (int)(total_bytes - 1);
  if (tpdu_len <= 0)
	  return;
  ...
  if (!sim_wait_prompt(uart_sim, 3000)){
    //dbg_puts(uart_dbg, "NO_PROMPT\r\n");
    return;
  }

  HAL_UART_Transmit(uart_sim, (uint8_t*)pdu, (uint16_t)strlen(pdu), 5000);
  uint8_t SUB=0x1A;
  HAL_UART_Transmit(uart_sim, &SUB, 1, 1000);

  // Hiện phản hồi uart để debug
 // sim_res_t r = sim_wait_result(uart_sim, 60000);
//  if (r == SIM_RES_OK)
//	  dbg_puts(uart_dbg, "OK\r\n");
//  else if (r == SIM_RES_CMS)
//	  dbg_puts(uart_dbg, "CMS ERROR\r\n");
...
//  else
//	  dbg_puts(uart_dbg, "TIMEOUT\r\n");
  sim_wait_result(uart_sim, 60000);
}
